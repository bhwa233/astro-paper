...
const HEADING_EMPHASIS_SELECTOR = ["h1", "h2", "h3", "h4", "h5", "h6"]
  .flatMap(heading => [`${heading} strong`, `${heading} b`])
  .join(", ");
...
export function htmlNodeToMarkdown(
  ...
  source
    .querySelectorAll(HEADING_EMPHASIS_SELECTOR)
    .forEach(emphasis => emphasis.replaceWith(...emphasis.childNodes));
