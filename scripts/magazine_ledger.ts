...
import { compact, readJsonLedger, repoRoot, writeJsonLedger } from "./blog_common.ts";
...
function readLedger(file: string, keyPrefix: string): MagazineLedger {
  return readJsonLedger<MagazineLedger>(file, `${keyPrefix} issue ledger`, { version: 1, issues: [] }, raw => {
    const parsed = raw as Partial<MagazineLedger>;
