...
import {
  substackLedgerSchema,
  type SubstackLedger,
  type SubstackLedgerIssue,
...
export function readSubstackLedger(file: string): SubstackLedger {
  return readJsonLedger(
    file,
    "Substack translation ledger",
    { version: 1, issues: [] },
    raw => substackLedgerSchema.parse(raw)
  );
}

export function findSubstackIssue(
  ledger: SubstackLedger,
  canonicalUrl: string
): SubstackLedgerIssue | undefined {
  return ledger.issues.find(issue => issue.canonicalUrl === canonicalUrl);
}

export function upsertSubstackIssue(
  file: string,
  issue: SubstackLedgerIssue
): void {
  const ledger = readSubstackLedger(file);
  ledger.issues = [
    ...ledger.issues.filter(
      existing => existing.canonicalUrl !== issue.canonicalUrl
    ),
    issue,
  ].sort((left, right) =>
