...
import { importAtRuntime } from '../util/native-import.js'
...
async function loadSharp(): Promise<SharpModule> {
  // Resolved by Node rather than by Vite, and deliberately not a literal
  // specifier: the failure mode we want is a clear runtime error at the point of
  // use, not a resolution error for people who never publish images. See
  // util/native-import.ts.
  try {
    const module_ = await importAtRuntime<{ default: SharpModule }>('sharp')
