...
async function readConfigFile(root: string): Promise<Partial<ProjectConfig>> {
  for (const name of CONFIG_FILE_NAMES) {
    ...
    // Node's own import, not Vite's: the Astro integration reaches this during
    // `astro:build:done`, by which point Vite's module runner is closed. See
    // util/native-import.ts.
    const module_ = await importAtRuntime<{ default?: Partial<ProjectConfig> }>(
