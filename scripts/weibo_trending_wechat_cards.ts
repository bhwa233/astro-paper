...
import { fitWeiboTrendingWechatSummary, type WeiboTrendingWechatItem } from "./weibo_trending_wechat_compose.ts";

const BRAND = "微博热搜";
const THEME = PLATFORM_THEMES.weibo;
const INNER_WIDTH = 894;
const INNER_HEIGHT = 690;
...
function topicTree(item: WeiboTrendingWechatItem, index: number, total: number): SatoriNode {
  ...
  const summary = fitWeiboTrendingWechatSummary(item.summary);
  ...
  return platformCard(
    ...
    innerColumn([
        ...
        props: {
          ...
          children: [
            ...
            },
            {
              type: "div",
              props: {
                style: { display: "flex", flex: 1, fontSize: titleFontSize, lineHeight: 1.25, fontWeight: 700, overflow: "hidden" },
                children: title,
              },
            },
          ],
      ...
      { type: "div", props: { style: { display: "flex", width: "100%", height: "2px", margin: "28px 0 26px", background: "#E8E8E8" } } },
      {
        type: "div",
        props: {
          style: { display: "flex", flexGrow: 1, alignItems: "center", width: "100%", fontSize: summaryFontSize, lineHeight: 1.55, color: "#343434", overflow: "hidden" },
          children: summary,
        },
      },
      {
        type: "div",
        props: {
          style: { display: "flex", justifyContent: "flex-end", width: "100%", marginTop: "24px", fontSize: 24, color: "#9A9A9A" },
...
export async function renderWeiboTrendingWechatCards(date: string, items: WeiboTrendingWechatItem[]): Promise<Buffer[]> {
  ...
  const summaries = items.map(item => fitWeiboTrendingWechatSummary(item.summary));
