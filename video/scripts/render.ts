...
import { parseVideoManifests } from "../src/contract.ts";
...
if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) throw new Error("usage: render.ts --date YYYY-MM-DD [--out-dir <directory>] [--result-json <file>]");
...
process.stderr.write(`[reddit-life-video] bundling ${manifests.length} videos for ${date}\n`);
...
for (const [position, manifest] of manifests.entries()) {
  ...
  const outputLocation = path.join(outputDir, `${index}.mp4`);
  ...
  process.stderr.write(`[reddit-life-video] rendering ${index}/${manifests.length} (${composition.durationInFrames} frames) -> ${outputLocation}\n`);
  await renderMedia({
    composition,
    serveUrl,
    codec: "h264",
    ...
    onBrowserDownload,
    onProgress: ({ progress }) => {
      if (process.stderr.isTTY) process.stderr.write(`\r[reddit-life-video] ${index}/${manifests.length} ${Math.round(progress * 100)}%`);
    },
  });
  if (process.stderr.isTTY) process.stderr.write("\n");
  videos.push({ index, title: manifest.title, question: manifest.question, outputLocation, durationInFrames: composition.durationInFrames, cards: manifest.cards.length });
}

const result = { date, videos };
